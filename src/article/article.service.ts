    ...
    if (query.favorited) {
      ...
      if (authorFavoriteArticlesIds.length) {
        queryBuilder.andWhere(
          'articles.id IN (:...authorFavoriteArticlesIds)',
          { authorFavoriteArticlesIds },
        );
      } else {
  ...
  async updateArticle(
    ...
    if (authorId !== currentUserId) {
      throw new HttpException(
        'You dont have rights to update this article',
  ...
  async getArticlesPython(): Promise<any[]> {
    try {
      ...
      if (error.response?.data?.detail) {
        throw new HttpException(
          error.response?.data?.detail || 'Error from Articlelytics Service',
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
      } else {
        throw new HttpException(
          "Can't connect to Articlelytics Service",
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
      }
    }
